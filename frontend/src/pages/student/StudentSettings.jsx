...
import { User, Lock, Bell, Moon, Save, GraduationCap, Building, Hash } from 'lucide-react';
...
export default function StudentSettings() {
  ...
  const [profile, setProfile] = useState({
    name: '', rollNumber: '', usn: '', semester: '', department: '',
  ...
  useEffect(() => {
    studentAPI.getSettings().then(({ data }) => {
      const u = data.data;
      setProfile({
        name: u.name || '', rollNumber: u.rollNumber || '', usn: u.usn || '',
  ...
  const saveProfile = async (e) => {
    ...
    setSaving(true);
    try {
      const { data } = await studentAPI.updateSettings(profile);
      toast.success('Profile updated successfully');
      login(useAuthStore.getState().token, data.data);
    } catch (err) {
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input type="text" name="semester" value={profile.semester} onChange={handleProfileChange} className="input pl-10" />
                </div>
              </div>
              <div>
                ...
                <div className="relative">
                  ...
                  <input type="text" name="rollNumber" value={profile.rollNumber} onChange={handleProfileChange} className="input pl-10" />
                </div>
              </div>
              <div>
                <label className="block text-xs font-semibold text-gray-400 mb-1.5">USN</label>
                <div className="relative">
                  <Hash className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-500" />
                  <input type="text" name="usn" value={profile.usn} onChange={handleProfileChange} className="input pl-10" />
                </div>
              </div>
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-gray-500">Receive exam results & alerts</p>
                </div>
